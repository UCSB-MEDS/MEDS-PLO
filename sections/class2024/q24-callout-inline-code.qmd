```{r in-line code q24a}
# total respondents ----
q24a_num_answers_pre <- meds2024_before_clean |>
  select(reflec_spec) |>
  count() |>
  pull()

q24a_num_answers_post <- meds2024_after_clean |>
  select(reflec_spec) |>
  count() |>
  pull()

# total not answer 1 ----
q24a_not1_pre <- meds2024_before_clean |>
  select(reflec_spec) |>
...
q24a_not1_post <- meds2024_after_clean |>
  select(reflec_spec) |>
  group_by(reflec_spec) |>
  count() |>
  ungroup() |>
...
# answered 1 ----
q24a_1_pre <- meds2024_before_clean |>
  select(reflec_spec) |>
  group_by(reflec_spec) |>
  count() |>
  ungroup() |>
...
q24a_1_post <- meds2024_after_clean |>
  select(reflec_spec) |>
  group_by(reflec_spec) |>
  count() |>
  ungroup() |>
