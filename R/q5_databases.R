...
clean_q5_databases_bothPP <- function(PLO_data_clean){
  ...
  df <- PLO_data_clean |> 
    ...
    # select necessary cols ----
    select(databases, timepoint) |> 
    
    # sum ----
    group_by(timepoint, databases) |> 
    count() |> 
    ungroup() 
  ...
  for (i in 1:length(options)){
    ...
    if (cat_name %in% pull(pre_meds[,2])) {
      ...
      message(cat_name, " does not exist. Adding now.")
      new_row <- data.frame(timepoint = "Pre-MEDS", databases = cat_name, n = 0)
  ...
  for (i in 1:length(options)){
    ...
    if (cat_name %in% pull(post_meds[,2])) {
      ...
      message(cat_name, " does not exist. Adding now.")
      new_row <- data.frame(timepoint = "Post-MEDS", databases = cat_name, n = 0)
  ...
  all_q5_data <- rbind(pre_meds, post_meds)
