...
clean_q4_prog_lang_bothPP <- function(PLO_data_clean){
  ...
  df <- PLO_data_clean |> 
    ...
    # select necessary cols ----
    select(program_lang, timepoint) |> 
    
    # sum ----
    group_by(timepoint, program_lang) |> 
    count() |> 
    ungroup() 
  ...
  for (i in 1:length(options)){
    ...
    if (cat_name %in% pull(pre_meds[,2])) {
      ...
      message(cat_name, " does not exist. Adding now.")
      new_row <- data.frame(timepoint = "Pre-MEDS", program_lang = cat_name, n = 0)
  ...
  pre_meds <- pre_meds |> 
    mutate(program_lang = fct_relevel(program_lang,
                                      c("Never", "Less than once per year", 
  ...
  for (i in 1:length(options)){
    ...
    if (cat_name %in% pull(post_meds[,2])) {
      ...
      message(cat_name, " does not exist. Adding now.")
      new_row <- data.frame(timepoint = "Post-MEDS", program_lang = cat_name, n = 0)
  ...
  all_q4_data <- rbind(pre_meds, post_meds)
