...
clean_q6_version_control_bothPP <- function(PLO_data_clean){
  ...
  df <- PLO_data_clean |> 
    ...
    # select necessary cols ----
  select(version_control, timepoint) |> 
    
    # sum ----
  group_by(timepoint, version_control) |> 
    count() |> 
    ungroup() 
  ...
  for (i in 1:length(options)){
    ...
    if (cat_name %in% pull(pre_meds[,2])) {
      ...
      message(cat_name, " does not exist. Adding now.")
      new_row <- data.frame(timepoint = "Pre-MEDS", version_control = cat_name, n = 0)
  ...
  for (i in 1:length(options)){
    ...
    if (cat_name %in% pull(post_meds[,2])) {
      ...
      message(cat_name, " does not exist. Adding now.")
      new_row <- data.frame(timepoint = "Post-MEDS", version_control = cat_name, n = 0)
  ...
  all_q6_data <- rbind(pre_meds, post_meds)
